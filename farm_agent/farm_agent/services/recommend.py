...
def _template(
    ...
    lines.extend(["", "## 提案（MVP・参考）", _heuristics(crop, weather, ndvi, soil_proxy)])
    lines.extend(
        [
            "",
...
def _heuristics(crop: str, weather: Any, ndvi: Any | None, soil_proxy: Any | None) -> str:
    tips: list[str] = []
...
def generate_recommendation(
    ...
    base = _template(crop, geo_name, weather, ndvi, soil_proxy)
    if not key:
    ...
    try:
        ...
        return Recommendation(text=base + "\n\n## LLM追記\n" + content, source="openai+gpt-4o-mini")
    except Exception:
